...
class DataExporter:
    ...
    @staticmethod
    def export_convergence_data(convergence_data):
        """导出算法收敛数据到CSV文件"""
        # 确保数据目录存在
        os.makedirs(DATA_DIR, exist_ok=True)
        ...
        with open(convergence_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["迭代次数", "温度", "当前成本", "最佳成本"])
            
            for i, (temp, current, best) in enumerate(zip(
                convergence_data["temperatures"],
                convergence_data["current_costs"],
                convergence_data["best_costs"]
            )):
                writer.writerow([i, temp, current, best])

        logger.info(f"算法收敛数据已导出至 {convergence_path}")