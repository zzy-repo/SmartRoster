...
class Shift:
    def __init__(
        self, day, start_time, end_time, required_positions, store
    ):  # 添加store参数
        self.day = day
...
def generate_neighbor(current_schedule):
    ...
    # 修改：只选择同一门店的员工
    candidates = [
        e for e in employees if e.position == selected_pos and e.store == shift.store
    ]
    if candidates:
...
shifts = [
    # 旗舰店常规班次
    Shift(
        0, "08:00", "12:00", {"门店经理": 1, "副经理": 1, "店员（导购）": 6}, "旗舰店"
    ),
    Shift(
        0,
        "12:00",
        "18:00",
        {"副经理": 1, "店员（收银）": 4, "店员（导购）": 8},
        "旗舰店",
    ),
    Shift(
        0,
