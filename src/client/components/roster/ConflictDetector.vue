...
// 检查时间重叠冲突
const checkTimeOverlap = (currentShift: ScheduleShift, _employee: any) => {
  if (!rosterStore.schedule) return
  ...
  for (const shift of employeeShifts) {
    // 跳过当前正在检查的班次
    if (shift.id === currentShift.id) continue
...
// 检查休息时间不足冲突
const checkRestHours = (currentShift: ScheduleShift, _employee: any) => {
  if (!rosterStore.schedule) return
...
// 检查每日工作时长超限
const checkDailyHours = (currentShift: ScheduleShift, employee: any) => {
  if (!rosterStore.schedule || !employee.preferences) return
...
// 检查每周工作时长超限
const checkWeeklyHours = (currentShift: ScheduleShift, employee: any) => {
  if (!rosterStore.schedule || !employee.preferences) return
...
// 检查连续工作天数超限
const checkConsecutiveDays = (currentShift: ScheduleShift, _employee: any) => {
  if (!rosterStore.schedule) return
