...
/**
 * @api {delete} /api/schedule/rules/:id 删除排班规则
 * @apiName DeleteScheduleRule
 * @apiGroup Schedule
 * @apiDescription 删除指定的排班规则
 * 
 * @apiParam {Number} id 规则ID
 * 
 * @apiSuccess {String} message 成功消息
...
app.delete('/rules/:id', async (req, res) => {
  try {
    ...
    // 强制删除ID为1的规则
    if (id !== '1') {
      return res.status(400).json({ error: '只能删除ID为1的规则' })
    }
